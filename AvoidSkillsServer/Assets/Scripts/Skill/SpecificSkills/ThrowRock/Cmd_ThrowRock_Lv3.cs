...
public class Cmd_ThrowRock_Lv3 : SkillCommand
{
    ...
    public override void cmd(Player _player, PlayerStatus _status, Vector3 _mousePos)
    {
        ...
        ob1.GetComponent<Projectile>().Initialize(_player.id, 2f, skillInfo.skillCode, skillInfo.level);
        ob2.GetComponent<Rigidbody>().velocity = velocity;
        ob2.GetComponent<Projectile>().Initialize(_player.id, 2f, skillInfo.skillCode, skillInfo.level);
        ob3.GetComponent<Rigidbody>().velocity = velocity;
        ob3.GetComponent<Projectile>().Initialize(_player.id, 2f, skillInfo.skillCode, skillInfo.level);
    }
}
